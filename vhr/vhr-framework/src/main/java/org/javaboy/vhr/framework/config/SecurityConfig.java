...
public class SecurityConfig {
     ...
     * 1. HttpSession
     * 2. SecurityContextHolder：这个本质上是一个
     * ThreadLocal，这个的特点是在哪个线程中存入的数据，就在哪个线程读取，换一个线程就读取不出来了。
     * 一般的流程是这样：
     * 登录成功之后，虽然信息存入到了 SecurityContextHolder 中，在 Spring Security
     * 后续的执行流程中，凡是需要获取当前用户信息的时候，都从 SecurityContextHolder 中获取，而不会
     * 从 HttpSession 获取（因为并非所有系统都会选择把信息存入到 HttpSession，例如有时候是 JWT 登录，信息可能存在 Redis
     * 中），当登录请求结束的时候，会将 SecurityContextHolder 中
     * 的信息清除。下一个请求到达的时候，会先从 HttpSession 中读取登录用户信息，然后存入到 SecurityContextHolder
     * 中，这样在后续的流程中，就可以使用 SecurityContextHolder 中的信息了
     *
     * @return
     */
    @Bean
            ...
            return new AuthorizationDecision(granted);
                ...
                .formLogin(f -> f.usernameParameter("username")
                        ...
                        .successHandler((req, resp, auth) -> {
                            ...
                            principal.setPassword(null);
                            resp.getWriter()
                                    .write(new ObjectMapper().writeValueAsString(RespBean.ok("登录成功", principal)));
                ...
                .logout(logout -> {
                    logout
                            ...
                            .logoutSuccessHandler((req, resp, auth) -> {
                                ...
                                principal.setPassword(null);
                                resp.getWriter()
                                        .write(new ObjectMapper().writeValueAsString(RespBean.ok("注销成功", principal)));
                            });
                });
